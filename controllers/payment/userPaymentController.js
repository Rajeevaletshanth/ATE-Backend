...
const getPaymentCustomerId = async (user_id) => {
    try {
...
module.exports = {
        ...
        try {
            await Card.findAll({
                ...
                if(response.length > 0){
                    const customerId = await getPaymentCustomerId(response[0].user_id);
    ...
    direct_payment: async (req, res) => {
        ...
        try {
            await Card.findAll({
                ...
                if(response.length > 0){
                    const customerId = await getPaymentCustomerId(user_id);
    ...
    capture_payment: async (req, res) => {
        const { user_id, amount, paymentMethodId, currency } = req.body
        try {
            const customer_id = await getPaymentCustomerId(user_id);
            if(customer_id){
                const paymentIntent = await stripe.paymentIntents.create({
                amount,
